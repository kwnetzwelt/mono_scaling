...
namespace Scaling
{
	internal class Lanczos
	{
		...
		public SColor[] Filter(int _targetWidth, int _targetHeight) {
			...
			// reset kernel cache
			this.m_kernelsCacheX = new Kernel1D[100]; 
			this.m_kernelsCacheY = new Kernel1D[100]; 
		...
		private double lanczos(double support, double d) {
			...
			double t = d * System.Math.PI;
			return support*System.Math.Sin(t)*System.Math.Sin(t/support)/(t*t);
		}
		...
		private SColor fastconvolve( int x, int y, Kernel1D kernelx, Kernel1D kernely) {
			...
			float g = (float)System.Math.Min(1,System.Math.Max(0,gc));
			float b = (float)System.Math.Min(1,System.Math.Max(0,bc));
			float a = (float)System.Math.Min(1,System.Math.Max(0,ac));
